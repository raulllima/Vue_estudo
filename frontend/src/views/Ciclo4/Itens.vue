<template>
  <div class="mainPage">
    ...
    >
      ...
      <template #item-allow_analise="item">
        <InputCheckbox
          class="inputCheckbox"
          v-model="item.isAllowAnalysis"
          :uniqueId="'cbx-' + item.id"
          @update:modelValue="() => isAllowAnalysis(item)"
        />
      </template>
      ...
      <template #item-type_item="item">
        <label>
          <select
            @change="(e) => typeEdit('type_item', item, e.target.value)"
      ...
      <template #expand="item">
        ...
        >
          ...
          <template #item-desmenbrado="item">
            <div
              class="input-container"
              @keydown.enter="btnEditItem(item)"
              @keydown.esc="disableEdit(item)"
            ...
            >
              <input
                ...
                type="text"
                :readonly="item.isReadonly"
                :value="item.desmenbrado"
                @copy.prevent
                @input="(e) => updateItem(item, 'desmenbrado', e.target.value)"
              />
              <div class="underline"></div>
            </div>
          </template>
          ...
          <template #item-custo="item">
            ...
            >
              <input
                ...
                :value="item.custo"
                @copy.prevent
                @input="(e) => updateItem(item, 'custo', e.target.value)"
              />
              <div class="underline"></div>
            </div>
          </template>
          ...
          <template #item-fornecedor="item">
            ...
            >
              <input
                ...
                @copy.prevent
                @input="(e) => updateItem(item, 'fornecedor', e.target.value)"
              />
              <div class="underline"></div>
            </div>
          </template>
          ...
          <template #item-material_organico="item">
            <label>
              <select
                @change="
                  (e) => typeEdit('material_organico', item, e.target.value)
                "
                :disabled="item.isReadonly"
              >
                <option
                  value="Sim"
                  :selected="item.material_organico === 'Sim'"
                >
                  Sim
                </option>
                <option
                  value="Não"
                  :selected="item.material_organico === 'Não'"
                >
                  Não
                </option>
              </select>
          ...
          <template #item-nocivo_saude="item">
            <label>
              <select
                @change="(e) => typeEdit('nocivo_saude', item, e.target.value)"
                :disabled="item.isReadonly"
              >
                <option value="Sim" :selected="item.nocivo_saude === 'Sim'">
                  Sim
                </option>
                <option value="Não" :selected="item.nocivo_saude === 'Não'">
                  Não
                </option>
              </select>
            </label>
          ...
          <template #item-nocivo_ambiente="item">
            <label>
              <select
                @change="
                  (e) => typeEdit('nocivo_ambiente', item, e.target.value)
                "
                :disabled="item.isReadonly"
              >
                <option value="Sim" :selected="item.nocivo_ambiente === 'Sim'">
                  Sim
                </option>
                <option value="Não" :selected="item.nocivo_ambiente === 'Não'">
                  Não
                </option>
              </select>
            </label>
          ...
          <template #item-vendavel="item">
            <label>
              <select
                @change="(e) => typeEdit('vendavel', item, e.target.value)"
                :disabled="item.isReadonly"
              >
                <option value="Sim" :selected="item.vendavel === 'Sim'">
                  Sim
                </option>
                <option value="Não" :selected="item.vendavel === 'Não'">
                  Não
                </option>
              </select>
            </label>
...
const headers = [
  { text: "ID", value: "id" },
  { text: "HABILITAR ANALISE", value: "allow_analise" },
...
export default {
  ...
  setup() {
    ...
    const isAllowAnalysis = (item) => {
      items.value = items.value.map((i) =>
        i.id === item.id
          ? {
              ...i,
              isAllowAnalysis: item.isAllowAnalysis === 1 ? 0 : 1,
            }
          : i
      );

      api.put("/itens/C4/editItem", {
        data: item,
      });
    };

    const typeEdit = (key, item, newValue) => {
      items.value = items.value.map((i) =>
        i.id === item.id
          ? {
              ...i,
    ...
    const btnDeleteItem = (val) => {
      api.delete("/itens/C4/deleteItem/" + val.id).then(() => {
