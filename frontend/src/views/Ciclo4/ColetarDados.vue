  ...
  <div class="demo-page">
    <div class="demo-page-navigation">
      <nav>
        <ul>
          ...
          <li>
            <a v-scroll-to="'#simpleMenuOption3'">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
    ...
    <main class="demo-page-content">
      <section>
        ...
        <div class="nice-form-group">
          <label>Desmembrado</label>
          <input
            v-model="desmembrado"
            type="text"
            placeholder="Desmembrado"
            value=""
            maxlength="50"
          />
        </div>
        <div class="nice-form-group">
          <label>Compos. química</label>
          <input
            v-model="composQuimica"
            type="text"
            placeholder="Compos. química"
            value=""
            maxlength="50"
          />
        </div>
        ...
        <div class="nice-form-group">
          ...
          <input
            v-model="demandaMensal"
            type="text"
            placeholder="Demanda mensal"
            value=""
            data-js="money"
          />
        </div>
        ...
        <div class="nice-form-group">
          <label>Aplic. comercial</label>
          <input
            v-model="aplicComercial"
            type="text"
            placeholder="Aplic. comercial"
            value=""
            data-js="money"
          />
        </div>
        ...
        <div class="nice-form-group">
          <label>Mat. Orgânico?</label>
          <select v-model="matOrganico">
            <option option disabled selected hidden>Selecione uma opção</option>
            <option>Sim</option>
            <option>Não</option>
          </select>
        </div>
        ...
        <div class="nice-form-group">
          <label>Nocivo À Saúde?</label>
          <select v-model="nocivoASaude">
            <option option disabled selected hidden>Selecione uma opção</option>
            <option>Sim</option>
            <option>Não</option>
          </select>
        ...
        <div class="nice-form-group">
          <label>Noc. Ambiente?</label>
          <select v-model="nocAmbiente">
            <option option disabled selected hidden>Selecione uma opção</option>
            <option>Sim</option>
            <option>Não</option>
          </select>
      ...
      <section>
        ...
        <fieldset class="nice-form-group">
          <div class="nice-form-group">
            <input
              @change="changeTypeItem('Renovável')"
              type="checkbox"
              id="check-3"
              class="switch"
              v-model="typeItem"
              value="Renovável"
            />
            <label for="check-3">Renovável</label>
          </div>

          <div class="nice-form-group">
            <input
              @change="changeTypeItem('Não Renovável')"
              type="checkbox"
              id="check-4"
              class="switch"
              v-model="typeItem"
              value="Não Renovável"
            />
            <label for="check-4">Não Renovável</label>
          </div>

          <div class="nice-form-group">
            <input
              @change="changeTypeItem('Reciclável')"
              type="checkbox"
              id="check-5"
              class="switch"
              v-model="typeItem"
              value="Reciclável"
            />
            <label for="check-5">Reciclável</label>
          </div>

          <div class="nice-form-group">
            <label>UP Cycling</label>
            <select v-model="upCycling">
              <option option disabled selected hidden>
                Selecione uma opção
              </option>
              <option>Sim</option>
              <option>Não</option>
        ...
        <details>
          <summary>
            <div @click="createItem" class="toggle-code">Cadastrar Item</div>
          </summary>
        </details>
      </section>
...
export default {
  ...
  setup() {
    ...
    const descricao = ref("");
    const material = ref("");
    const desmembrado = ref("");
    const composQuimica = ref("");
    const quantidade = ref("");
    const unidade = ref("");
    const custo = ref("");
    const demandaMensal = ref("");
    const fornecedor = ref("");
    const aplicComercial = ref("");

    const matOrganico = ref("Selecione uma opção");
    const nocivoASaude = ref("Selecione uma opção");
    const nocAmbiente = ref("Selecione uma opção");
    const vendavel = ref("Selecione uma opção");
    const destino = ref("");

    const typeItem = ref(["Renovável"]);
    const upCycling = ref("Selecione uma opção");
    const downCycling = ref("Selecione uma opção");
    ...
    const nextPage = () => {
      ...
      store.dispatch("updateViewAnalisarValidar", false);
    };
    ...
    const createItem = () => {
      api
        ...
        .catch((err) => {
          notify({
            title: "Notificação",
            text: "Erro ao criar um novo item! " + err,
    ...
    const updateItemData = () => {
      requestBody.value = {
        cod_item: codItem.value,
        descricao: descricao.value,
        material: material.value,
        desmembrado: desmembrado.value,
        compos_quimica: composQuimica.value,
        quantidade: quantidade.value,
        unidade: unidade.value,
        custo: custo.value,
        demanda_mensal: demandaMensal.value,
        fornecedor: fornecedor.value,
        aplic_comercial: aplicComercial.value,
        material_organico: matOrganico.value,
        nocivo_saude: nocivoASaude.value,
        nocivo_ambiente: nocAmbiente.value,
        vendavel: vendavel.value,
        destino: destino.value,
        type_item: typeItem.value[typeItem.value.length - 1],
        up_cycling: upCycling.value,
        down_cycling: downCycling.value,
      };
    };

    const changeTypeItem = (name) => {
      typeItem.value = [];
      typeItem.value.push(name);
    };

    watch(upCycling, () => {
      if (upCycling.value === "Sim") {
        downCycling.value = "Não";
      } else {
        downCycling.value = "Sim";
      }
    });

    watch(downCycling, () => {
      if (downCycling.value === "Sim") {
        upCycling.value = "Não";
      } else {
        upCycling.value = "Sim";
      }
    ...
    watch(
      ...
      { immediate: true } // Atualiza itemData imediatamente na montagem do componente
    );
    return {
      nextPage,
      pageItens,
