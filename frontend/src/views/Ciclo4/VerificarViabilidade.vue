<template>
  <div class="demo-page">
    <div class="demo-page-navigation">
      <nav>
        <ul>
          ...
          <li>
            <a v-scroll-to="'#simpleMenuOption1'">
              <svg
          ...
          <li>
            <a v-scroll-to="'#simpleMenuOption2'">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
    ...
    <main class="demo-page-content">
      <section>
        <div class="href-target" id="simpleMenuOption1"></div>
        <h1>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
        ...
        <div class="nice-form-group">
          <label>TMA</label>
          <small
            >% mínimo de retorno que o projeto deve gerar para ser aceito</small
          >
          <input
            v-model="fieldTMA"
            type="text"
        ...
        <div class="nice-form-group">
          <label>Ganho</label>
          <small
            >Venda, aluguel ou redução de custo com o compartilhamento do
            produto</small
          >
          <input v-model="fieldGanho" type="number" placeholder="Ganho" />
        </div>

        <div class="nice-form-group">
          <label>Demanda</label>
          <small
            >Demanda mensal de vendas venda, aluguel ou redução de custo</small
          >
          <input v-model="fieldDemanda" type="number" placeholder="Demanda" />
        </div>
        ...
        <div class="nice-form-group">
          ...
          <input
            ...
            :value="fieldInvestimentos ? 'R$ ' + fieldInvestimentos : ''"
            data-js="money"
            readonly
          />
        </div>
        <div class="nice-form-group">
          ...
          <input
            type="text"
            placeholder="Custo"
            :value="fieldCustos ? 'R$ ' + fieldCustos : ''"
            data-js="money"
            readonly
          />
        </div>
      ...
      <section>
        <div class="href-target" id="simpleMenuOption2"></div>
        <h1>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
        ...
        <fieldset class="nice-form-group">
          <div class="nice-form-group">
            <label>VPL</label>
            <input v-model="fieldVPL" type="text" placeholder="VPL" disabled />
          </div>
          <div class="nice-form-group">
            <label>TIR</label>
            <input
              class="defineColor"
              :value="fieldTIR ? fieldTIR + ' %' : ''"
              type="text"
              placeholder="TIR"
              disabled
            />
          ...
          <div class="nice-form-group">
            <label>Payback Simples</label>
            <input
              v-model="fieldPaybackSimples"
              type="text"
              placeholder="PAYBACK"
              disabled
            />
          ...
          <div class="nice-form-group">
            <label>Payback Descontado</label>
            <input
              v-model="fieldPaybackDescontado"
              type="text"
              placeholder="PAYBACK"
              disabled
            />
...
// header
const fieldTMA = ref();
const fieldGanho = ref();
const fieldDemanda = ref();
const fieldInvestimentos = ref();
const fieldCustos = ref();

// part 2
const fieldVendaMensal = ref();
const fieldCustoMensal = ref();
const fieldLucroMensal = ref();

// part 1
const fieldVPL = ref();
const fieldTIR = ref();
const fieldPaybackSimples = ref();
const fieldPaybackDescontado = ref();

// Define color of field "TIR"
const defineColorTIR = () => {
  const ColorTIR = document.getElementsByClassName("defineColor");
  if (fieldTIR.value > fieldTMA.value) {
    ColorTIR[0].style.color = "green";
  } else {
    ColorTIR[0].style.color = "red";
  }
};
...
export default {
  ...
  setup() {
    ...
    watch([fieldGanho, fieldDemanda], (newValue) => {
      fieldVendaMensal.value = newValue[0] * newValue[1];
    });

    watch(fieldVendaMensal, (newValue) => {
      // Remove Mask
      let rawCustoMensal = parseFloat(unformat(fieldCustos.value, configMask));

      let rawInvestimentos = "";
      api
        .get("/itens/C4/getAllItems")
        .then((response) => {
          const data = response.data;

          rawInvestimentos = data.reduce((total, item) => {
            return (
              total +
        ...
        .then(() => {
          // This is 'lucro mensal'
          fieldLucroMensal.value = rawCustoMensal + rawInvestimentos + newValue;
        });
    });

    watch(
      [fieldLucroMensal, fieldTMA, fieldGanho, fieldDemanda],
      (newValue) => {
        class Indicators {
          constructor() {}
          ...
          CalcFluxoDeCaixa() {
            ...
            let lucroMensal = parseFloat(fieldLucroMensal.value);

            let investimentoInicial = parseFloat(
              unformat(fieldInvestimentos.value, configMask)
            );

            fluxo.push(-investimentoInicial);

            for (let i = 0; i < 5; i++) {
              fluxo.push(lucroMensal);
            }
          ...
          CalcTMA() {
            return parseFloat(fieldTMA.value) / 100;
          }

          CalcVPL() {
            let vpl = 0;
            for (let t = 0; t < this.CalcFluxoDeCaixa().length; t++) {
              vpl +=
                this.CalcFluxoDeCaixa()[t] / Math.pow(1 + this.CalcTMA(), t);
            }
            fieldVPL.value = vpl.toFixed(2);
          }

          CalcTIR() {
            let tir = 0.1; // Inicializando com 10%
            let precisao = 0.00001; // Precisão do cálculo (5 casas decimais)
            ...
            do {
              ...
              // Obtém os fluxos de caixa
              const fluxosDeCaixa = this.CalcFluxoDeCaixa();

              // Calcula o valor do VPL com a TIR atual
              for (let t = 0; t < fluxosDeCaixa.length; t++) {
                fTir += fluxosDeCaixa[t] / Math.pow(1 + tir, t);
                fDerivadaTir +=
                  (-t * fluxosDeCaixa[t]) / Math.pow(1 + tir, t + 1);
              }

              // Verifica se a derivada é muito pequena
              if (Math.abs(fDerivadaTir) < 1e-10) {
                break; // ou faça um ajuste na TIR
              }

              diferenca = fTir / fDerivadaTir;
              tir -= diferenca;

              iteracao++;
            } while (Math.abs(diferenca) > precisao && iteracao < maxIteracoes);

            // Atribuição do resultado ao campo
            fieldTIR.value = (tir * 100).toFixed(2); // Formata para 5 casas decimais em percentual
          }

          calcularVP(taxa, periodos, FV) {
            return FV / Math.pow(1 + taxa, periodos);
          }

          calcFluxoVP(fluxo, TMA) {
            return fluxo.map((valor, i) => this.calcularVP(TMA, i, valor));
          }

          calcAcumulado(fluxoVP) {
            let acumulado = [];
            fluxoVP.reduce((soma, valorAtual, i) => {
              acumulado[i] = soma + valorAtual;
              return acumulado[i];
            }, 0);
            return acumulado;
          }
          ...
          calcPayback(TMA) {
            const fluxoVP = this.calcFluxoVP(this.CalcFluxoDeCaixa(), TMA);
            const acumulado = this.calcAcumulado(fluxoVP);

            // Encontrar o ano em que o acumulado se torna positivo
            let anoPositivo = acumulado.findIndex((valor) => valor > 0);

            if (anoPositivo === -1) {
              return "Payback não atingido"; // Não atinge o payback
            }

            // Cálculo do Payback exato com interpolação linear
            return (
              anoPositivo -
              1 +
              Math.abs(acumulado[anoPositivo - 1]) / fluxoVP[anoPositivo]
            ).toFixed(2);
          }
        }
        ...
        if (
          fieldTMA.value > 0 &&
          fieldGanho.value > 0 &&
          fieldDemanda.value > 0
        ) {
          const indicators = new Indicators();
          indicators.CalcVPL();
          indicators.CalcTIR();

          fieldPaybackSimples.value = indicators.calcPayback(0);
          fieldPaybackDescontado.value = indicators.calcPayback(
            indicators.CalcTMA()
          );
        } else {
          fieldVPL.value = "";
          fieldTIR.value = "";
          fieldPaybackSimples.value = "";
          fieldPaybackDescontado.value = "";
        }
    ...
    const nextPage = () => {
      router.beforeEach(async (to, from, next) => {
        if (to.meta.shouldFetch) {
          const data = {
            Investimentos: fieldInvestimentos.value,
            Custos: fieldCustos.value,
            TMA: fieldTMA.value,
            VPL: fieldVPL.value,
            TIR: fieldTIR.value,
            PaybackSimples: fieldPaybackSimples.value,
            PaybackDescontado: fieldPaybackDescontado.value,
            Indices: route.meta.data,
          };
          to.meta.data = data;
        }
        next();
      ...
      store.dispatch("updateViewImplantarRedesenhar", false);
    };
    ...
    return {
      ...
      fieldTIR,
      fieldPaybackSimples,
      fieldPaybackDescontado,
    };
  },
  mounted() {
    this.runOnMounted();

    api.get("/investimentos/C4/getAllItems").then((response) => {
      if (response.status !== 200) return {};

      fieldInvestimentos.value = format(
    ...
    api.get("/custos/C4/getAllItems").then((response) => {
      if (response.status !== 200) return {};

      fieldCustos.value = format(
        response.data.reduce((total, item) => {
          return total + (item.total || 0);
        }, 0) * -1,
        configMask
    ...
    api.get("/itens/C4/getAllItems").then((response) => {
      if (response.data.length === 0) return {};

      fieldCustoMensal.value = response.data.reduce((total, item) => {
        return (
          total + parseFloat(item.custo_unit) * parseInt(item.qtd_compartilhada)
        );
      }, 0);
    });
  },
