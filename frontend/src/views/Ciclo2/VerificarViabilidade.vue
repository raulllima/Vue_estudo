...
export default {
  ...
  setup() {
    ...
    class Indicators {
      ...
      CalcVPL() {
        ...
        for (let t = 0; t < this.CalcFluxoDeCaixa().length; t++) {
          vpl += this.CalcFluxoDeCaixa()[t] / Math.pow(1 + this.CalcTMA(), t);
        }
      ...
      CalcTIR() {
        ...
        do {
          ...
          for (let t = 0; t < fluxosDeCaixa.length; t++) {
            ...
            fDerivadaTir += (-t * fluxosDeCaixa[t]) / Math.pow(1 + tir, t + 1);
          }
    ...
    const calculateIndicators = () => {
      if (fieldCustos.value && fieldInvestimentos.value) {
        if (
          fieldTMA.value > 0 &&
          fieldGanho.value > 0 &&
    ...
    watch(fieldVendaMensal, (newValue) => {
      ...
      let rawInvestimentos = parseFloat(
        unformat(fieldInvestimentos.value, configMask)
      );
    ...
    watch([fieldLucroMensal, fieldTMA, fieldGanho, fieldDemanda], () => {
      calculateIndicators();
    });
  ...
  mounted() {
    try {
      api.get("/investimentos/C2/getAllItems").then((response) => {
      ...
      api.get("/custos/C2/getAllItems").then((response) => {
        if (response.status !== 200) return {};
      ...
      api.get("/itens/C2/getAllItems").then((response) => {
