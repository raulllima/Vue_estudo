<template>
      ...
      <main class="demo-page-content">
        <section>
          ...
          <h1>
            ...
            Definir Fluxo - Etapa 2
          </h1>
          <p>Cadastro dos Itens</p>

          <div class="nice-form-group">
            <small>
              <img id="image-ciclo" src="../assets/images/fluxo2.png" alt="" />
            </small>
          </div>

          <!-- <div class="nice-form-group">
            <label> Para resposta 6 positiva, qual ou quais? </label>
            <fieldset class="nice-form-group">
              <div class="nice-form-group">
                <input type="checkbox" id="check-1" />
                <label for="check-1"> Ciclo 1: Manter / Prolongar </label>
              </div>

              <div class="nice-form-group">
                <input type="checkbox" id="check-2" />
                <label for="check-2"> Ciclo 2: Reutilizar / Redistribuir</label>
              </div>

              <div class="nice-form-group">
                <input type="checkbox" id="check-3" />
                <label for="check-3"> Ciclo 3: Remanufaturar</label>
              </div>

              <div class="nice-form-group">
                <input type="checkbox" id="check-4" />
                <label for="check-4"> Ciclo 4: Reciclar</label>
              </div>
            </fieldset>
          </div> -->
          ...
          <div class="nice-form-group">
            <label>
              Qual ciclo técnico está mais alinhado com
              a atividade circular que a empresa estudar e por que?
            </label>

            <fieldset class="nice-form-group">
              <div class="nice-form-group">
                <input v-model="question8Answer" value="C1" type="radio" name="radio" id="r-1" />
                <label for="r-1"> Ciclo 1: Manter / Prolongar</label>
              </div>

              <div class="nice-form-group">
                <input v-model="question8Answer" value="C2" type="radio" name="radio" id="r-2" />
                <label for="r-2"> Ciclo 2: Reutilizar / Redistribuir </label>
              </div>

              <div class="nice-form-group">
                <input v-model="question8Answer" value="C3" type="radio" name="radio" id="r-3" />
                <label for="r-3"> Ciclo 3: Remanufaturar</label>
              </div>

              <div class="nice-form-group">
                <input v-model="question8Answer" value="C4" type="radio" name="radio" id="r-4" />
                <label for="r-4"> Ciclo 4: Reciclar</label>
              </div>
            </fieldset>
          </div>
        </section>
  ...
  <div id="content-modal-ciclo" class="content-modal-ciclo">
    <span class="close-modal-ciclo">&times;</span>
    <img class="image-modal-ciclo" id="image-modal-ciclo">
  </div>
</template>
...
export default {
  name: "coletarDadosCiclo1",
  setup() {
    const router = useRouter();
    const store = useStore();

    const question6Answer = ref(store.state.question6Answer);
    const question7Answer = ref(store.state.question7Answer);
    const question8Answer = ref(store.state.question8Answer);

    watch(question6Answer, (newValue) => {
      store.dispatch("updateQuestion6Answer", newValue);
    });
    watch(question7Answer, (newValue) => {
      store.dispatch("updateQuestion7Answer", newValue);
    });
    watch(question8Answer, (newValue) => {
      store.dispatch("updateQuestion8Answer", newValue);
      store.dispatch("updateCycle", newValue.toLocaleLowerCase());
    });

    const nextPage = () => {
      router.push({
        name: question8Answer.value + "-ColetarDados",
      });
      store.dispatch('updateViewColetarDados', false)
    };
  ...
  components: {
    ...
    var modal = document.getElementById("content-modal-ciclo");
    var img = document.getElementById("image-ciclo");
    var modalImg = document.getElementById("image-modal-ciclo");
    var closeBtn = document.getElementsByClassName("close-modal-ciclo")[0];
