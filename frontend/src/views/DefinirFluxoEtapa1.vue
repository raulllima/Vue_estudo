<template>
  <form @submit.prevent="handleSubmit">
    <div class="demo-page">
      <div class="demo-page-navigation">
        <nav>
          <a class="btnFinish" href="#" @click="nextPage">
            <img :src="srcBtnIcon" alt="" />

            {{ question5Answer === "Técnicos" ? "Continuar" : "Encerrar" }}
          </a>
        </nav>
      </div>
      <main class="demo-page-content">
        <section>
          ...
          <h1>
            ...
            Definir Fluxo - Etapa 1
          </h1>
          <p>Questionário</p>

          <div class="nice-form-group">
            <label>Sua empresa é um fabricante de um produto próprio?</label>
            <select v-model="question1Answer">
          ...
          <div class="nice-form-group">
            <label>Você concorda que diante de um cenário futuro de um planeta de
              recursos finitos, a economia circular não é opcional e sim
              inevitável?</label>
            <select v-model="question2Answer">
              <option disabled selected hidden>Selecione uma opção</option>
              <option>Sim</option>
              <option>Não</option>
            </select>
          ...
          <div class="nice-form-group">
            <label>A sua empresa realiza projetos alinhados com a Economia
              Circular?</label>
            <select v-model="question3Answer">
              <option disabled selected hidden>Selecione uma opção</option>
              <option>Sim</option>
              <option>Não</option>
            </select>
          ...
          <div class="nice-form-group">
            <label>Para resposta 3 positiva, mencionar quais projetos foram
              realizados?</label>
            <textarea rows="5" value="" :disabled="checkQuestion3" />
          </div>

          <div class="nice-form-group">
            <label>Sua empresa já possui algum ciclo técnico implantado?
            </label>
            <small>
              <img id="image-premissas" src="../assets/images/premissas.png" alt="" />
            </small>
            <select v-model="question5Answer">
              <option disabled selected hidden>Selecione uma opção</option>
  ...
  <div id="content-modal-premissas" class="content-modal-premissas">
    <span class="close-modal-premissas">&times;</span>
    <img class="image-modal-premissas" id="image-modal-premissas">
  </div>
</template>
...
export default {
  name: "DefinirFluxoEtapa1",
  setup() {
    const router = useRouter();
    const store = useStore();

    const srcBtnIcon = ref("/exit.svg");
    const checkQuestion3 = ref(true)

    const allAnswer = ref([])
    const question1Answer = ref(store.state.question1Answer);
    const question2Answer = ref(store.state.question2Answer);
    const question3Answer = ref(store.state.question3Answer);
    const question4Answer = ref(store.state.question4Answer);
    const question5Answer = ref(store.state.question5Answer);

    watch(question1Answer, (newValue) => {
      store.dispatch("updateQuestion1Answer", newValue);
      allAnswer.value.push({ newValue });
    });
    watch(question2Answer, (newValue) => {
      store.dispatch("updateQuestion2Answer", newValue);
      allAnswer.value.push({ newValue });
    });
    watch(question3Answer, (newValue) => {
      store.dispatch("updateQuestion3Answer", newValue);
      allAnswer.value.push({ newValue });
      newValue === "Sim" ? checkQuestion3.value = false : checkQuestion3.value = true;
    });
    watch(question4Answer, (newValue) => {
      allAnswer.value.push({ newValue });
      store.dispatch("updateQuestion4Answer", newValue);
    });
    watch(question5Answer, (newValue) => {
      allAnswer.value.push({ newValue });
      store.dispatch("updateQuestion5Answer", newValue);
    });

    watch(question5Answer, (newValue) => {
      if (newValue === "Técnicos") {
        srcBtnIcon.value = "/continue.svg";
      } else {
        srcBtnIcon.value = "/exit.svg";
      }
    });

    watch(allAnswer, (newAllAnswer) => {
      console.log(newAllAnswer)
    })

    const nextPage = () => {
      if (question5Answer.value === "Técnicos") {
        router.push({
  ...
  mounted() {
    // Pegando a imagem e a modal
    var modal = document.getElementById("content-modal-premissas");
    var img = document.getElementById("image-premissas");
    var modalImg = document.getElementById("image-modal-premissas");
    var closeBtn = document.getElementsByClassName("close-modal-premissas")[0];
    ...
    closeBtn.onclick = function () {
      ...
      modalImg.classList.remove('zoomed'); // Remove o zoom ao fechar
    }

    // Fechar a modal ao clicar fora da imagem
    ...
    // Função de zoom in/out ao clicar na imagem dentro do modal
...
.content-modal-premissas {
  ...
  overflow: hidden; /* Impede que a imagem aumentada saia dos limites do modal */
}

.image-modal-premissas {
  width: 80%;
  max-width: 700px;
  max-height: 80%; /* Limita a altura para não sair da tela */
  transition: transform 0.25s ease;
  cursor: zoom-in;
}

.image-modal-premissas.zoomed {
  transform: scale(2); /* Aumenta a escala da imagem para 2x */
  cursor: zoom-out;
}

.close-modal-premissas {
  position: absolute;
  top: 20px;
  right: 35px;
  color: #fff;
