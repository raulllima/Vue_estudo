...
db.serialize(() => {
    db.run(`
    CREATE TABLE IF NOT EXISTS InvestimentosC2 (
    ...
    `, (err) => {
        if (err) {
            console.error(err)
            console.error("[FAIL] Tabela 'InvestimentosC2'.")
        } else {
            console.log("[OK] Tabela 'InvestimentosC2'.")
        }
    });
});
...
module.exports = {
    async count(req, res) {
        db.get(`
            SELECT seq FROM sqlite_sequence WHERE name = 'InvestimentosC2'
            `, [], (err, row) => {
    ...
    async getItem(req, res) {
        ...
        db.get(`
            SELECT * FROM InvestimentosC2 WHERE id = ?
        `, [id], (err, row) => {
            if (err) {
                res.status(500).send({
                    "code": 500,
    ...
    async deleteItem(req, res) {
        ...
        db.run(`
            DELETE FROM InvestimentosC2 WHERE id = ?
        `, [id], (err) => {
            if (err) {
                return res.status(500).send({
                    "code": 500,
