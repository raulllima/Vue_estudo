...
db.serialize(() => {
    db.run(`
    CREATE TABLE IF NOT EXISTS InvestimentosC3 (
    ...
    `, (err) => {
        if (err) {
            console.error(err)
            console.error("[FAIL] Tabela 'InvestimentosC3'.")
        } else {
            console.log("[OK] Tabela 'InvestimentosC3'.")
        }
    });
});
...
module.exports = {
    async count(req, res) {
        db.get(`
            SELECT seq FROM sqlite_sequence WHERE name = 'InvestimentosC3'
            `, [], (err, row) => {
    ...
    async getItem(req, res) {
        ...
        db.get(`
            SELECT * FROM InvestimentosC3 WHERE id = ?
        `, [id], (err, row) => {
            if (err) {
                res.status(500).send({
                    "code": 500,
    ...
    async deleteItem(req, res) {
        ...
        db.run(`
            DELETE FROM InvestimentosC3 WHERE id = ?
        `, [id], (err) => {
            if (err) {
                return res.status(500).send({
                    "code": 500,
