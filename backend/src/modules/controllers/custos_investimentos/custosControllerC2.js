...
db.serialize(() => {
    db.run(`
    CREATE TABLE IF NOT EXISTS CustosC2 (
    ...
    `, (err) => {
        if (err) {
            console.error(err)
            console.error("[FAIL] Tabela 'CustosC2'.")
        } else {
            console.log("[OK] Tabela 'CustosC2'.")
        }
    });
});
...
module.exports = {
    async count(req, res) {
        db.get(`
            SELECT seq FROM sqlite_sequence WHERE name = 'CustosC2'
            `, [], (err, row) => {
