...
db.serialize(() => {
    db.run(`
    CREATE TABLE IF NOT EXISTS CustosC1 (
    ...
    `, (err) => {
        if (err) {
            console.error(err)
            console.error("[FAIL] Tabela 'CustosC1'.")
        } else {
            console.log("[OK] Tabela 'CustosC1'.")
        }
    });
});
...
module.exports = {
    async count(req, res) {
        db.get(`
            SELECT seq FROM sqlite_sequence WHERE name = 'CustosC1'
            `, [], (err, row) => {
