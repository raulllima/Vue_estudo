...
db.serialize(() => {
    db.run(`
    CREATE TABLE IF NOT EXISTS CustosC4 (
    ...
    `, (err) => {
        if (err) {
            console.error(err)
            console.error("[FAIL] Tabela 'CustosC4'.")
        } else {
            console.log("[OK] Tabela 'CustosC4'.")
        }
    });
});
...
module.exports = {
    async count(req, res) {
        db.get(`
            SELECT seq FROM sqlite_sequence WHERE name = 'CustosC4'
            `, [], (err, row) => {
